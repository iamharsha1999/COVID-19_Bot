...
for i,st in enumerate(st_names):
    # print(st)
    if st == 'Punjab':
        plt.text(x_list[i]-2,y_list[i]-0.2,st, fontsize=10,fontweight = 'extra bold', color = 'orange')
    elif st == "Chandigarh":
        plt.text(x_list[i]+0.5,y_list[i],st, fontsize=10,fontweight = 'extra bold', color = 'orange')
    elif st == 'Himachal Pradesh':
        plt.text(x_list[i]+0.3,y_list[i]-0.2,st, fontsize=10,fontweight = 'extra bold', color = 'orange')
    elif st == 'Haryana':
        plt.text(x_list[i]-2.5,y_list[i]-0.2,st, fontsize=10,fontweight = 'extra bold', color = 'orange')
    elif st == 'Kerala':
        plt.text(x_list[i]-2,y_list[i]-0.2,st, fontsize=10,fontweight = 'extra bold', color = 'orange')
    else:
        plt.text(x_list[i]+0.5,y_list[i]-0.2,st, fontsize=10,fontweight = 'extra bold', color = 'orange')
    plt.scatter(x_list[i],y_list[i],c = 'r', s = 200, marker=r"$ {} $".format(confirmedCases[i]))
